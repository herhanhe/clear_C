...
/*练习菜单*/
typedef enum { Term, KeyPos, KeyPosComp, Clang, Conversation, InValid }
Menu;
...
char *cstr[] = {
	...
	"setloacle","setvbuf",	"signal",	"sin",		"sinh",			"sprintf",
...
char *vstr[] = {
	...
	"I hope I'm not disturbing you.",		/*希望不会打扰到你。*/
...
void pos_training2(void){
	...
	for (i = 0;i < 4;i++) {
		...
		printf(format,i+1,k + 1,kstr[k],	k + 2,kstr[k + 1],
						  k + 3,kstr[k + 2],k + 4,kstr[k + 1]);
	}
	/*不重复选择块（最多16个）*/
	sno = 0;
	while (1) {
		printf("编号（结束选择为50/停止练习99）：");
		do {
			scanf("%d",&temp);
			if (temp == 99) return;			/*停止练习*/
...
/*C语言/英文会话训练*/
void word_training(const char *mes,char *str[],int n){
	int stage;
	int qno,pno;					/*题目编号和上一次的题目编号*/
	int tno,mno;					/*字符数量和错误次数*/
	clock_t start,end;				/*开始时间和结束时间*/
